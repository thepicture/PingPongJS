...
class InGameSound {
    static play(elementId, isLoop = false, volume = 0.8) {
...
class ScoreTable {
    ...
    static setScore(player, enemy) {
        ...
        score.innerHTML = `Player: ${this.playerScore}<br>Enemy: ${this.enemyScore}`;
    }

    static addScore(deltaPlayer, deltaEnemy) {
        this.playerScore += deltaPlayer;
        this.enemyScore += deltaEnemy;
        this.setScore(this.playerScore, this.enemyScore);
    }
...
class InputChecker {
    static heldKeySet = new Set();

    static connect() {
        document.addEventListener('keydown', (event) => {
            event.preventDefault();
            this.heldKeySet.add(event.code);
        });
        document.body.addEventListener('keyup', (event) => {
            event.preventDefault();
            this.heldKeySet.delete(event.code);
        });
    }
    ...
    static processInput() {
        ...
        } else if (this.heldKeySet.has('Escape')) {
...
class Ball {
    ...
    drawShadow() {
        ...
        ballShadow.style.zIndex = this.ballElement.style.zIndex - 1;
